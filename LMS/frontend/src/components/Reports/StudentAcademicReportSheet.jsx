...
const StudentAcademicReportSheet = ({ data, studentName }) => {
    ...
    return (
        <div className="bg-card  p-8 rounded-xl border border-border/50 shadow-none overflow-hidden flex flex-col space-y-6 my-8 animate-slide-up">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h3 className="text-2xl font-semibold text-foreground tracking-tight">Academic Report Sheet</h3>
                    <p className="text-xs font-semibold text-muted-foreground tracking-wide mt-1 opacity-60">Official Transcript</p>
                </div>
                    ...
                    <tbody className="divide-y divide-border/50 text-sm">
                        {data.map((item, index) => (
                            <tr key={index} className="hover:bg-primary/5 transition-colors group">
                                <td className="px-6 py-5 font-semibold text-foreground tracking-tight border-r border-border/30">
                                    {item.className}
                                </td>
                                ...
                                <td className="px-6 py-5 text-center">
                                    <span className={`font-semibold ${item.averagePercentage >= 70 ? 'text-emerald-500' :
                                        item.averagePercentage >= 50 ? 'text-amber-500' : 'text-rose-500'}`}>
                                        {item.averagePercentage}%
                                    </span>
                                </td>
                                <td className="px-6 py-5 text-center">
                                    <span className={`font-semibold ${(item.attendance?.percentage || 0) >= 75 ? 'text-emerald-500' : 'text-muted-foreground'}`}>
                                        {item.attendance?.percentage || 0}%
                                    </span>
                                </td>
                                <td className="px-6 py-5 text-center">
                                     {item.averagePercentage >= 50 ? (
                                         <span className="text-xs font-semibold tracking-wide text-emerald-500 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl shadow-none">Pass</span>
                                     ) : (
                                         <span className="text-xs font-semibold tracking-wide text-rose-500 bg-rose-500/10 border border-rose-500/20 px-3 py-1.5 rounded-xl shadow-none">Fail</span>
                                     )}
                                 </td>
                            </tr>
                        ))}
            ...
            <div className="p-5 bg-muted rounded-xl flex justify-between items-center text-sm border border-border/50">
                <span className="text-xs font-semibold tracking-wide text-muted-foreground opacity-60">Official Auto-Generated Transcript</span>
                <span className="text-xs font-semibold tracking-[0.3em] text-primary shadow-none">OFFICIAL</span>
            </div>
        </div>
    );
};
