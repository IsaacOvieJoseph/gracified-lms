...
// ── Modes ─────────────────────────────────────────────────────────────────────
const MODES = {
  topic: { label: 'Generate Topic', icon: BookOpen, color: 'from-violet-600 ', endpoint: '/ai/generate-topic', resultKey: 'topic' },
  assignment: { label: 'Generate Assignment', icon: FileText, color: 'from-blue-600 ', endpoint: '/ai/generate-assignment', resultKey: 'assignment' },
  exam: { label: 'Generate Exam', icon: FileText, color: 'from-rose-600 ', endpoint: '/ai/generate-exam', resultKey: 'exam' },
  classroom: { label: 'Generate Class', icon: School, color: 'from-emerald-600 ', endpoint: '/ai/generate-classroom', resultKey: 'classroom' },
  powerpoint: { label: 'Generate Slides', icon: Presentation, color: 'from-amber-600 ', endpoint: '/ai/generate-powerpoint', resultKey: 'presentation' },
  qna: { label: 'Q&A Assistant', icon: HelpCircle, color: 'from-amber-500 ', endpoint: '/ai/qna-assistant', resultKey: 'qna' },
  syllabus: { label: 'Generate Syllabus', icon: BookOpen, color: 'from-indigo-600 ', endpoint: '/ai/generate-syllabus', resultKey: 'syllabus' },
};

// ── Field component ───────────────────────────────────────────────────────────
const Field = ({ label, children }) => (
  <div className="space-y-1.5">
    <label className="text-xs font-semibold tracking-wide text-slate-400">{label}</label>
    {children}
  </div>
);
...
const ResultViewer = ({ mode, result, onApply, onDownloadPptx, downloadingPptx }) => {
    ...
    return (
      <div className="space-y-3 animate-in slide-in-from-bottom-4">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-semibold text-primary tracking-wide">✨ AI Generated Topic</h4>
        ...
        <button
          ...
          className="w-full py-3 bg-primary text-white rounded-xl font-semibold text-xs tracking-wide hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-none dark:shadow-none"
        >
  ...
  if (mode === 'syllabus') {
    ...
    return (
      <div className="space-y-4 animate-in slide-in-from-bottom-4">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-semibold text-primary tracking-wide">✨ AI Generated Syllabus</h4>
          <span className="text-xs font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded-full">{topics.length} Topics</span>
        </div>
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
          {topics.map((t, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-4 shadow-none">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-6 h-6 rounded-xl bg-primary/10 dark:bg-indigo-950/30 flex items-center justify-center text-xs font-semibold text-primary">
                  {idx + 1}
                </div>
                <p className="font-semibold text-slate-900 dark:text-white text-sm">{t.name}</p>
                <div className="ml-auto flex items-center gap-1 text-xs font-semibold text-primary bg-primary/10 dark:bg-indigo-950/30 px-2 py-0.5 rounded-full">
              ...
              {t.lessonsOutline && (
                <div className="text-xs text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl italic">
        ...
        <button
          ...
          className="w-full py-4 bg-primary text-white rounded-xl font-semibold text-xs tracking-wide hover:opacity-90 transition-all flex items-center justify-center gap-3 shadow-none dark:shadow-none"
        >
        ...
        <div className="max-h-48 overflow-y-auto space-y-2 pr-1">
          {questions.slice(0, 5).map((q, i) => (
            <div key={i} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-800">
              ...
              {q.options && <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Options: {q.options.map((opt, idx) => <span key={idx}>{idx > 0 && ' / '}<MathText text={opt} /></span>)}</p>}
            </div>
          ))}
          {questions.length > 5 && <p className="text-xs text-slate-400 text-center">+{questions.length - 5} more questions</p>}
        </div>
        <button
          onClick={() => onApply(result)}
  ...
  if (mode === 'powerpoint') {
    ...
    return (
      <div className="space-y-3 animate-in slide-in-from-bottom-4">
        <h4 className="text-xs font-semibold text-amber-600 tracking-wide">✨ AI Generated Presentation</h4>
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-4 space-y-2">
          <p className="font-semibold text-slate-900 dark:text-white">{result.presentationTitle}</p>
          <p className="text-sm text-slate-600 dark:text-slate-400">{result.subtitle}</p>
          <p className="text-xs font-semibold text-amber-600 dark:text-amber-400">{slides.length} Slides</p>
        </div>
        <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
          {slides.map((slide, i) => (
            <div key={i} className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-none">
              <div className="flex items-center gap-2 mb-1">
                <span className="w-5 h-5 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 rounded text-xs font-semibold flex items-center justify-center">{slide.slideNumber}</span>
                <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{slide.title}</p>
                <span className="ml-auto text-[8px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded">{slide.type}</span>
              </div>
              {slide.bulletPoints?.length > 0 && (
                <ul className="space-y-0.5 ml-7">
                  {slide.bulletPoints.map((bp, j) => (
                    <li key={j} className="text-xs text-slate-500 dark:text-slate-400 flex gap-1"><span className="text-amber-400">•</span>{bp}</li>
                  ))}
                </ul>
              )}
              {slide.speakerNotes && (
                <p className="text-xs text-slate-400 italic mt-1 ml-7 border-t border-slate-50 dark:border-slate-800 pt-1">📝 {slide.speakerNotes}</p>
              )}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl font-semibold text-xs tracking-wide hover:bg-slate-200 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-2"
          >
  ...
  if (mode === 'classroom') {
    return (
      <div className="space-y-3 animate-in slide-in-from-bottom-4">
        <h4 className="text-xs font-semibold text-emerald-600 tracking-wide">✨ AI Generated Class Details</h4>
        <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/50 rounded-xl p-5 space-y-4">
          <div>
            <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 tracking-wide mb-1">Class Name</p>
            <p className="font-semibold text-slate-900 dark:text-white">{result.name}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 tracking-wide mb-1">Description</p>
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{result.description}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 tracking-wide mb-1">Learning Outcomes</p>
  ...
  if (mode === 'qna') {
    return (
      <div className="space-y-3 animate-in slide-in-from-bottom-4">
        <h4 className="text-xs font-semibold text-amber-600 tracking-wide">✨ AI Assistant Answer</h4>
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-5 space-y-4">
          ...
          {result.suggestedFollowUp && result.suggestedFollowUp.length > 0 && (
            <div className="pt-3 border-t border-amber-200 dark:border-amber-800/50">
              <p className="text-xs font-semibold text-amber-600 dark:text-amber-400 tracking-wide mb-2">Suggested Follow-ups</p>
              <div className="flex flex-wrap gap-2">
                {result.suggestedFollowUp.map((f, i) => (
                  <span key={i} className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800 px-2 py-1 rounded-xl text-xs font-semibold text-amber-700 dark:text-amber-400">{f}</span>
                ))}
              </div>
            </div>
          )}
...
const AIAssistantPanel = ({
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 bg-slate-900/40 " style={{ zIndex: 9998 }} onClick={onClose} />

      {/* Panel */}
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white dark:bg-slate-950 shadow-none flex flex-col overflow-hidden animate-in slide-in-from-right-8 duration-300 border-l dark:border-slate-800" style={{ zIndex: 9999 }}>
        {/* Header */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xs text-violet-200 font-semibold tracking-wide">Powered by</p>
                  {provider === 'groq' && (
                    <span className="flex items-center gap-1 text-xs font-semibold text-white bg-primary/50 px-2 py-0.5 rounded-full">
                  ...
                  {!provider && <span className="text-xs text-violet-200 font-semibold tracking-wide">AI</span>}
                </div>
              </div>
            </div>
            ...
            {allowedModes.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold tracking-wider transition-all ${mode === m ? 'bg-white text-violet-700 shadow-none' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}
                >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-primary text-white rounded-xl font-semibold tracking-wide text-sm hover:opacity-90 transition-all shadow-none dark:shadow-none flex items-center justify-center gap-3 disabled:opacity-60"
          >
            {loading ? (
