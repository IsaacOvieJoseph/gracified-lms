...
const GracyQuizModal = ({ quizConfig, onClose, onNewQuiz }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-950 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Practice with Gracy</h2>
            {quiz && (
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-8 custom-scrollbar">
          {generating ? (
            ...
            <div className="max-w-2xl mx-auto space-y-8">
              ...
              {/* Results Card */}
              {result && (
                <div className="bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-800 rounded-2xl p-6 text-center animate-in slide-in-from-top-4">
                  <div className="text-4xl font-black text-sky-600 dark:text-sky-400 mb-2">
                    {result.score} <span className="text-xl text-sky-400/50">/ {result.total}</span>
                  </div>
                  <p className="font-bold text-sky-700 dark:text-sky-300 mb-4">
                  ...
                  <div className="mt-6">
                    <button 
                      onClick={onNewQuiz}
                      className="px-6 py-2.5 bg-sky-500 hover:bg-sky-600 text-white rounded-xl font-bold shadow-md shadow-sky-500/20 transition-all active:scale-95"
                    >
                      New Quiz
                    </button>
                  </div>
              ...
              <div className="space-y-6">
                {quiz.questions.map((q, qIndex) => {
                  ...
                  return (
                    <div key={qIndex} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
                      <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-4 flex gap-3">
                      ...
                      <div className="space-y-3 pl-7">
                        {q.options.map((option, oIndex) => {
                          ...
                          if (isReviewed) {
                            ...
                            if (selected) {
                              optClass += "bg-sky-50 dark:bg-sky-900/20 border-sky-300 dark:border-sky-700 text-sky-800 dark:text-sky-200 font-medium ring-1 ring-sky-500";
                              icon = <div className="w-5 h-5 rounded-full border-4 border-sky-500 shrink-0 mt-0.5" />;
                            } else {
                              optClass += "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-sky-300 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer";
                              icon = <div className="w-5 h-5 rounded-full border-2 border-slate-300 dark:border-slate-600 shrink-0 mt-0.5" />;
                            }
                          }
              ...
              {!result && (
                <div className="pt-4 pb-8 flex flex-col items-center">
                  ...
                  <button
                    onClick={submit}
                    disabled={submitting}
                    className="w-full max-w-sm py-3.5 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white rounded-xl font-bold shadow-lg shadow-sky-500/25 transition-all active:scale-95 disabled:opacity-70 flex items-center justify-center gap-2"
                  >
                    {submitting ? (
