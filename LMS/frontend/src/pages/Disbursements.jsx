            ...
            <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="flex gap-4 mb-6 border-b border-gray-200">
                    <button
                        ...
                        className={`pb-2 px-4 text-sm font-medium transition-colors ${activeTab === 'pending' ? 'border-b-2 border-indigo-600 text-primary' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Pending Approval ({pendingPayouts.length})
                    </button>
                    <button
                        onClick={() => setActiveTab('history')}
                        className={`pb-2 px-4 text-sm font-medium transition-colors ${activeTab === 'history' ? 'border-b-2 border-indigo-600 text-primary' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Payout History
                    </button>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-xl shadow-none border border-gray-100 overflow-hidden overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Class Owner</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Bank Details</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Payment Details</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Payment Ref</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Deductions</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500">Net Payout</th>
                                    {activeTab === 'history' && (
                                        <th className="px-6 py-4 text-xs font-semibold text-gray-500">Payout Ref</th>
                                    )}
                                        ...
                                        <td className="px-6 py-4">
                                            {activeTab === 'pending' ? (
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-primary text-white text-xs font-semibold rounded-xl hover:bg-primary/90 transition disabled:opacity-50 flex items-center gap-2"
                                                >
